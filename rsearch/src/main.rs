...
use crate::rdkit::{AromaticityModel, SanitizeFlags};
...
pub mod rdkit {
    ...
    impl ROMol {
        ...
        pub(crate) fn set_aromaticity(&mut self, mdl: AromaticityModel) {
            unsafe {
                rdkit_sys::RDKit_SetAromaticity(self.0, mdl.bits());
            }
        }

        pub(crate) fn assign_stereochemistry(&self) {
            unsafe {
                rdkit_sys::RDKit_AssignStereochemistry(self.0);
            }
        }

        pub(crate) fn add_hs(&self) {
            unsafe {
